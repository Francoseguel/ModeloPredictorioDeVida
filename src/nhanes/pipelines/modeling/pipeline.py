...
from .nodes import (
    train_classifiers, train_regressors, tune_classifiers, tune_regressors,
)

# Mismos datasets supervisados que data_split.
SUPERVISED = ["exam", "lab", "quest", "master"]
# Datasets que ademas pasan por tuning de hiperparametros (RandomizedSearchCV).
TUNED = ["master"]
...
def create_pipeline(**kwargs) -> Pipeline:
    ...
    for ds in TUNED:
        nodes.append(
            node(
                partial(tune_classifiers, ds_name=ds),
                inputs=[
                    f"{ds}_X_train", f"{ds}_y_train_clf",
                    f"{ds}_X_test", f"{ds}_y_test_clf",
                    "params:modeling",
                ],
                outputs=f"{ds}_clf_tuned_report",
                name=f"tune_clf_{ds}",
            )
        )
        nodes.append(
            node(
                partial(tune_regressors, ds_name=ds),
                inputs=[
                    f"{ds}_X_train", f"{ds}_y_train_reg",
                    f"{ds}_X_test", f"{ds}_y_test_reg",
                    "params:modeling",
                ],
                outputs=f"{ds}_reg_tuned_report",
                name=f"tune_reg_{ds}",
            )
        )
